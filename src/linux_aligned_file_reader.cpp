...
void execute_io(io_context_t ctx, int fd, std::vector<AlignedRead> &read_reqs, uint64_t n_retries = 0, uint64_t &total_requested_data_size)
{
#ifdef DEBUG
    for (auto &req : read_reqs)
    {
	...
	for (auto &req : read_reqs)
	{
		total_requested_data_size += req.len;
	}
    // break-up requests into chunks of size MAX_EVENTS each
    uint64_t n_iters = ROUND_UP(read_reqs.size(), MAX_EVENTS) / MAX_EVENTS;
    for (uint64_t iter = 0; iter < n_iters; iter++)
...
void LinuxAlignedFileReader::read(std::vector<AlignedRead> &read_reqs, io_context_t &ctx, bool async, uint64_t &total_requested_data_size)
{
    if (async == true)
    {
        diskann::cout << "Async currently not supported in linux." << std::endl;
    ...
    assert(this->file_desc != -1);
    execute_io(ctx, this->file_desc, read_reqs, total_requested_data_size);
}
